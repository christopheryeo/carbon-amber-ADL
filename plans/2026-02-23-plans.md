...
### 🔴 Priority 1: HIGH Severity Fixes (from Cross-Reference Report)
...
### 🟠 Priority 4: Planning Agent Fixes (from Functionality Review — Feb 23)

These address gaps in `planning.md` found by cross-referencing against `application.md` capabilities.

#### 4.1 🔄 Fix language detection → transcription dependency (Issue P1)
...
|----------|-------|---------------|
| 🔴 HIGH (runtime/validation fixes) | 6 | ~85 minutes |
| 🟡 MEDIUM (staleness/naming) | 7 | ~70 minutes |
| 🟠 Planning Agent fixes | 4 | ~75 minutes |
| 🟢 Cleanup/docs | 5 | ~35 minutes |
...
Items 1.1–1.3 and 2.4–2.5 all touch `memory.md`, so they should be done together in a single pass. Similarly, items 1.4–1.5 both touch `message_format.md`. Items 4.1–4.4 all touch `planning.md`.

**Suggested batches:**

1. **Batch A — `memory.md` overhaul** (1.1 + 1.2 + 1.3 + 2.4 + 2.5): Fix ID, overhaul example, remove Reviewer Agent, fix ARL naming, fix log paths
2. **Batch B — `message_format.md` updates** (1.4 + 1.5): Fix examples, add CAP-AUD-R
3. **Batch C — Delete stale files** (2.1 + 3.5): Remove placeholder agents, clean worktree
4. **Batch D — Update supporting docs** (2.2 + 2.6 + 3.3 + 3.4): Update instructions, template, README, cross-ref report
5. **Batch E — Schema updates** (1.6 + 2.7): Bring schema up to date
6. **Batch F — `planning.md` fixes** (4.1 + 4.2 + 4.3 + 4.4): Fix dependencies, complete tables, add example output
7. **Batch G — Minor fixes** (2.3 + 3.1 + 3.2): CAP-AUD-R format, goal.md numbering, memory concepts
