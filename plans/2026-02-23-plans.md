# Carbon Amber ADL — Improvement Plan for 23 February 2026
...
| Phase | Item | Status |
|-------|------|--------|
| Phase 1 | Implement `dispatch_agent` | ✅ Done (`dispatch.md` v1.0.0) |
| Phase 1 | Create unified action agent (was `execution_agent`) | ✅ Done (`action.md` v1.0.0, renamed Feb 23) |
| Phase 1 | Test single-task execution pipeline | ❌ Not yet tested |
| Phase 2 | Dispatch failure handling and retry logic | ✅ Included in `dispatch.md` |
| Phase 2 | Parallel execution group coordination | ✅ Included in `dispatch.md` |
| Phase 2 | Workflow state tracking | ✅ Included in `dispatch.md` |
| Phase 3 | Reposition reasoning_agent for synthesis | ✅ Done (`reasoning.md` v1.0.0) |
| Phase 3 | Deprecate perception_agent and action_agent placeholders | ❌ Placeholder files still exist |
| Phase 3 | Retire interpretation_agent | ❌ Placeholder file still exists |
| Phase 4 | Multi-task workflow testing | ❌ Not yet done |
| Phase 4 | Ref resolution validation | ❌ Not yet done |
...
#### 2.2 🆕 Overhaul `memory.md` output example (Findings D3, D5)
...
#### 2.3 🆕 Remove "Reviewer Agent" references (Finding A4)
...
#### 2.4 🆕 Update `message_format.md` examples (Findings D1, D2)
...
#### 2.5 🆕 Add CAP-AUD-R to Agent Chain Flow (Finding C1)
- **Problem:** Agent Chain Flow in `message_format.md` lists Action Agent capabilities but omits `CAP-AUD-R` (audience analysis)
- **Action:** Add `CAP-AUD-R` to the capability list at steps [4]–[5]
- **Files:** `context/governance/message_format.md`
- **Effort:** ~5 minutes

#### 2.6 🆕 Update `message_schema.json` to catch up with `message_format.md` (Findings E1, B1)
...
- **Action:** Audit changes from v1.3.0–v1.6.0 in `message_format.md` changelog and update the JSON schema accordingly
...
#### 3.1 🆕 Delete deprecated placeholder files (Finding B3)
...
#### 3.2 🆕 Update `instructions.md` (Finding B2)
...
#### 3.3 🆕 Standardise CAP-AUD-R identifier format (Finding A2)
- **Problem:** Inconsistent format — `application.md` uses `CAP-AUD-R001`, `goal.md` uses `CAP-AUD-Rxxx`, `dispatch.md` uses `CAP-AUD-R*`
...
#### 3.4 🆕 Fix `memory.md` orchestration naming (Finding A3)
...
#### 3.5 🆕 Fix `memory.md` log path convention (Finding D4)
...
#### 3.6 🆕 Update `application_template.md` (Finding B5)
...
#### 3.7 🆕 Add schema validation for capability IDs and DAG structure (Findings E4, E5, E6)
...
#### 4.1 🆕 Fix `goal.md` Common Mistakes numbering (Finding A5)
- **Problem:** Numbering is non-sequential (1–6, jumps to 10, 11, back to 7, 8, 9, then 12, 13)
- **Action:** Renumber sequentially
- **Files:** `agent/governance/goal.md`
- **Effort:** ~5 minutes

#### 4.2 🆕 Document or remove `memory.md` undefined concepts (Finding C3)
...
- **Action:** Add v2.4.0 entry documenting the Feb 21–23 changes
- **Files:** `README.md`
- **Effort:** ~10 minutes
...
### 🟠 Priority 5: Planning Agent Fixes (from Functionality Review — Feb 23)

These address gaps in `planning.md` found by cross-referencing against `application.md` capabilities.

#### 5.1 ✅ Fix language detection → transcription dependency (Issue P1)
- **Problem:** `planning.md` Dependency Rules table lists Transcription and Language Detection as independent parallel tasks. `application.md` mandates: `CAP-PRE-002 → CAP-AUD-004 → CAP-AUD-001` (extract audio → detect language → transcribe)
...
#### 5.2 🔄 Complete the weight table (Issue P2)
...
#### 5.3 🔄 Complete the dependency rules table (Issue P3)
...
#### 5.4 🔄 Add full workflow output to Example 2 (Issue P4)
...
#### 5.5 🆕 Enforce sequential derived_ref numbering (Log Validation — Feb 23)
- **Problem:** In log `20260223-1.md` Chain 1, the Planning Agent registered derived_1, derived_3, derived_4 — skipping derived_2. Per `message_format.md`, derived_ref IDs must follow sequential conventions (`derived_1`, `derived_2`, `derived_3`…) with no gaps.
...
#### 5.6 🆕 Fix Planning Agent message_id prefix convention (Log Validation — Feb 23)
...
#### 5.7 🆕 Log objective consolidation in deduplication_log (Log Validation — Feb 23)
- **Problem:** In log `20260223-1.md` Chains 1 and 2, the Goal Agent output contained 3 objectives, but the Planning Agent's input shows only 2 (objectives were merged/consolidated). However, the `deduplication_log` array is empty in both cases, meaning this consolidation is undocumented. This breaks audit transparency — there is no record of what was merged or why.
...
### 🔵 Priority 1: Agent-Application Separation Refactoring (TOP PRIORITY)
...
**Added to `CLAUDE.md`:** ✅ Architectural Principle section added.

#### Scan Results — Application-Specific Content Found in Agent Files
...
| `goal.md` | **LOW** | References to `application.md` Section 6 CAP-ID categories by name — this is acceptable since it's a reference, not a hardcoded definition. |
| `objective.md` | **LOW** | Needs verification — likely has video analysis examples. |
| `perception.md` | N/A | Deprecated placeholder — scheduled for deletion (item 2.1). |
| `interpretation.md` | N/A | Deprecated placeholder — scheduled for deletion (item 2.1). |

#### Refactoring Strategy

For each affected agent file, the approach is:

1. **Replace hardcoded task types with generic terminology** — e.g., "Audio extraction" → "Pre-processing task" or defer to `application.md` Section 6.9
2. **Replace hardcoded URIs with placeholder patterns** — e.g., `wasabi://dsta-bucket/...` → `{storage_backend}://...` or `storage://bucket/session-xxx/...`
3. **Replace platform names with generic references** — e.g., "YouTube URL" → "source URL" or "supported platform URL (see `application.md` Section 7)"
4. **Replace tool names with generic descriptions** — e.g., "ffmpeg" → "the tool mapped to this capability (see `application.md` Section 6)"
5. **Move CAP-to-tool mappings to `application.md`** — the mapping table in `action.md` belongs in `application.md`
6. **Keep examples functional but generic** — use placeholder CAP-IDs and abstract descriptions

#### Per-File Refactoring Items

##### 1.1 ✅ Refactor `planning.md` — Remove video-specific dependency/weight/ref rules
...
##### 1.2 ✅ Refactor `action.md` — Move tool mapping to `application.md`
- **Content to move:** CAP-to-MCP tool mapping table (lines ~108–130), Wasabi URIs, YouTube URLs
...
##### 1.3 ✅ Refactor `dispatch.md` — Genericise examples and retry scenarios
...
##### 1.4 ✅ Refactor `reasoning.md` — Genericise examples
...
##### 1.5 ✅ Refactor `memory.md` — Genericise knowledge examples
...
##### 1.7 ✅ Refactor `goal.md` — Move Patterns A–J to `application.md`
- **Action:** Move the video-specific Goal Decomposition Patterns to `application.md`. Update `goal.md` with a placeholder string to reference `application.md` Section N.
- **Files:** `agent/governance/goal.md`, `context/application.md`
- **Effort:** ~20 minutes

##### 1.8 🆕 Refactor `action.md` — Fix broken governance path and defaults
- **Action:** Fix `"agent/executional/execution.md"` → `"agent/executional/action.md"`. Move Step 4 default parameters to `application.md`.
- **Files:** `agent/executional/action.md`, `context/application.md`
- **Effort:** ~15 minutes

##### 1.9 🆕 Refactor `objective.md` and `goal.md` — Generalise scope validation
- **Action:** Replace video-specific scope validation lists with references to `application.md`. Remove generic terms like "video content".
- **Files:** `agent/governance/objective.md`, `agent/governance/goal.md`
- **Effort:** ~15 minutes

##### 1.10 🆕 Refactor `reasoning.md` — Move CAP-SYN output schemas
- **Action:** Move the detailed output schemas for synthesis tasks to `application.md`.
- **Files:** `agent/operational/reasoning.md`, `context/application.md`
- **Effort:** ~15 minutes

##### 1.11 🆕 Refactor `objective.md` — Generalise capability mapping
- **Action:** Replace capability mapping table with reference to `application.md`.
...
|----------|-------|---------------|
| 🔵 Agent-Application separation (P1) | 11 | ~225 minutes |
| 🔴 HIGH severity fixes (P2) | 6 | ~85 minutes |
| 🟠 MEDIUM severity fixes (P3) | 7 | ~70 minutes |
| 🟢 Cleanup/docs (P4) | 5 | ~35 minutes |
| 🟠 Planning Agent fixes (P5) | 7 | ~120 minutes |
| **Total** | **36 items** | **~9.0 hours** |

> **Note:** Priority 1 (agent-app separation) should be done FIRST. Priority 5 items 5.2–5.4 should be done AFTER Priority 1, so the planning agent tables are genericised rather than expanded with more application-specific data.
...
1. **Batch A — Agent-Application separation (Phase 1)** (1.1–1.6): Refactor all agent files to remove application-specific content. **START HERE.**
1b. **Batch A2 — Agent-Application separation (Phase 2)** (1.7–1.11): Apply deeper refactoring based on audit findings regarding scope validation, schemas, and pattern migrations.
2. **Batch B — `memory.md` overhaul** (2.1 + 2.2 + 2.3 + 3.4 + 3.5): Fix ID, overhaul example, remove Reviewer Agent, fix ARL naming, fix log paths
3. **Batch C — `message_format.md` updates** (2.4 + 2.5): Fix examples, add CAP-AUD-R
4. **Batch D — Delete stale files** (3.1 + 4.5): Remove placeholder agents, clean worktree
5. **Batch E — Update supporting docs** (3.2 + 3.6 + 4.3 + 4.4): Update instructions, template, README, cross-ref report
6. **Batch F — Schema updates** (2.6 + 3.7): Bring schema up to date
7. **Batch G — `planning.md` remaining fixes** (5.2 + 5.3 + 5.4 + 5.5 + 5.6 + 5.7): Complete tables and Example 2 using generic content (post-separation); fix derived_ref numbering gap, message_id prefix, and deduplication_log transparency
8. **Batch H — Minor fixes** (3.3 + 4.1 + 4.2): CAP-AUD-R format, goal.md numbering, memory concepts
